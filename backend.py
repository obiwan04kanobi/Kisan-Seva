...
from crop_ml import predict_crop  # crop_ml.py has a predict function
import os
import pandas as pd
...
app = Flask(__name__)
bcrypt = Bcrypt(app)
app.secret_key = '----removed for security----'  # Change this to a real secret in production

# MongoDB setup 
client = MongoClient('----removed for security----')  # Update the URI as per your MongoDB setup
db = client['KisanSeva']  # Database name
users = db['users']  # Collection for users
sellers = db['sellers']  # Collection for sellers
...
# Razorpay setup
razorpay_client = razorpay.Client(auth=("----removed for security----", "----removed for security----"))
...
def initialize_db():
    ...
    # Ensure the 'sellers' collection exists
    if 'sellers' not in db.list_collection_names():
...
@app.route('/submit_crop_selling', methods=['POST'])
def submit_crop_selling():
    ...
    if request.method == 'POST':
        ...
        fruit_quantity = request.form.get('fruit_quantity')
        vegetables = request.form.get('vegetables')
        vegetable_quantity = request.form.get('vegetable_quantity')
    ...
    return redirect(url_for('crop_selling'))
...
def scrollbar():
    ...
    predicted_crops = predict_crop(nitrogen, phosphorus, potassium, temperature, humidity, ph, rainfall)

    visitor_count = db.visitors.find_one({'page': 'index'})['count']
    # Render the template with the prediction (optional, can redirect to success page)
    return render_template("prediction.html", scrollbar_prediction=predicted_crops, visitor_count=visitor_count)


@app.route("/predict_dropdown", methods=["POST"])
def drop_down():
    # Load data from an Excel file
    # Ensure the Excel file is in the same directory as this script, or provide the full path
    df = pd.read_csv('/home/0221csds213/webapp/Hack-X/location_data.csv')

    # Extract user-selected location from the request
    selected_location = request.form.get("location")

    # Filter the DataFrame for the selected location
    if selected_location and selected_location in df['location'].values:
        location_data_row = df[df['location'] == selected_location].iloc[0]

    # Extract soil data from the DataFrame row
    nitrogen = location_data_row["nitrogen"]
    phosphorus = location_data_row["phosphorus"]
    potassium = location_data_row["potassium"]
    temperature = location_data_row["temperature"]
    humidity = location_data_row["humidity"]
    ph = location_data_row["ph"]
    rainfall = location_data_row["rainfall"]

    # Call your machine learning model (assuming predict_crop is defined elsewhere)
    predicted_crops = predict_crop(nitrogen, phosphorus, potassium, temperature, humidity, ph, rainfall)

    visitor_count = db.visitors.find_one({'page': 'index'})['count']
    # Render the template with the prediction
    return render_template("prediction.html", dropdown_prediction=predicted_crops, visitor_count=visitor_count)

@app.route('/loan')
def loan():
    visitor_count = db.visitors.find_one({'page': 'index'})['count']
    return render_template('loan.html', visitor_count=visitor_count)

@app.route('/crop_selling', methods=['GET'])
def crop_selling():
    if 'username' in session:
        ...
        if user:
            ...
            return render_template('crop_selling.html', user=user)
    else:
...
def search():
    visitor_count = db.visitors.find_one({'page': 'index'})['count']
    return render_template('prediction.html', visitor_count=visitor_count)
...
def admin_login():
    if request.method == "POST":
        username = request.form.get("username")
        password = request.form.get("password")
        if username == "admin" and password == "starwars":
            session['admin'] = True
...
@app.route('/admin_dashboard/<collection_name>')
def view_collection(collection_name):
    if 'admin' not in session:
        return redirect(url_for('admin_login'))
    collection_data = list(db[collection_name].find())
    return json.dumps(collection_data, default=json_util.default)


@app.route('/admin_dashboard/update/<collection_name>/<item_id>', methods=['POST'])
def update_item(collection_name, item_id):
    if 'admin' not in session:
        return redirect(url_for('admin_login'))
    updated_data = request.json
    db[collection_name].update_one({'_id': ObjectId(item_id)}, {'$set': updated_data})
    return jsonify({'status': 'success'})

@app.route('/admin_dashboard/delete/<collection_name>/<item_id>', methods=['POST'])
